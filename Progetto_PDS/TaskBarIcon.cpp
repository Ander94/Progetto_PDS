...
MainFrame::MainFrame(const wxString& title, class Settings* settings) : wxFrame(NULL, wxID_ANY, title)
{
	...
	if (boost::filesystem::is_regular_file("" + m_settings->getGeneralPath() + "profilo.png") != true) {
		m_settings->setImagePath("" + m_settings->getGeneralPath() + "user_default.png");
		if (boost::filesystem::is_regular_file("" + m_settings->getGeneralPath() + "user_default.png") != true) {
